...
public class Editor implements Runnable, ActionListener {
	...
	public static void main(String[] args) {
		try {	// hier setten, om problemen te voorkomen
	...
	private void pack() {
		filer.save();
		try {
			JarFile jar = FileSystem.pack(store.getActive().getPath(), store.getActive().get("id"));
	...
	private void unpack() {
		FileSystem.unpack(store.getActive().getPath());
